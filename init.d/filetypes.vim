autocmd FileType cpp        setlocal shiftwidth=4 tabstop=4 conceallevel=0
autocmd FileType html       setlocal shiftwidth=2 tabstop=2
autocmd FileType javascript setlocal shiftwidth=4 tabstop=4
autocmd FileType json       setlocal shiftwidth=2 tabstop=2 conceallevel=0
autocmd FileType vim        setlocal shiftwidth=2 tabstop=2 conceallevel=0
autocmd FileType python     setlocal shiftwidth=4 tabstop=4 wrap formatoptions+=t
autocmd FileType php        setlocal shiftwidth=2 tabstop=2 softtabstop=2 tabstop=2
autocmd FileType lua        setlocal shiftwidth=2 tabstop=2 softtabstop=2 tabstop=2
autocmd FileType text,help        setlocal nonumber norelativenumber

" ingat posisi terakhir cursor
autocmd BufReadPost *
    \ if line("'\"") > 1 && line("'\"") <= line("$") |
    \   execute "normal! g`\"" |
    \ endif
